...
def get_localized_text(data, lang_code: str) -> str:
    ...
    if text := data.get(lang_code): return text
    if text := data.get("de"): return text
    if data: return next(iter(data.values()))
    return "N/A"
    ...
    try:
        ...
        for col in json_cols:
            if col in df.columns:
                df[col] = df[col].apply(lambda x: json.loads(x) if x and isinstance(x, str) else (x if isinstance(x, (dict, list)) else {}))
        return df
...
# ADDED: "📚 Help" to the list of tabs
tab1, tab2, tab3, tab4 = st.tabs([
    T["tab_worklist"], 
    T["tab_overview"], 
    T["tab_inspector"], 
    T["tab_help"]  # <--- This was the hardcoded line
])
...
with tab3:
    ...
    dataset_map = {f"{row['display_title']} ({str(row['id'])[:8]}...)": row['id'] for _, row in filtered_df.iterrows()}
    ...
    if selected_display:
        ...
        with col_d2:
            ...
            if 'swiss_score' in record and record['swiss_score'] > 0:
                 st.info(f"Swiss FAIRC Score: {record['swiss_score']:.0f}")
            else: st.info("Deep quality checks pending.")

        with st.expander(T["inspector_raw"]):
            raw = record.to_dict()
            if 'display_title' in raw: del raw['display_title']
            if 'severity' in raw: del raw['severity']
            st.json(raw)
...
with tab4:
    ...
    # Updated Table with FAIRC (Contextuality Added)
    table_md = f"""
